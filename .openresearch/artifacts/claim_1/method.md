...
Finite non-negative-mean cases are checked against a rigorous Chernoff
log-sum-exp upper bound on `OPT`. For each Chernoff parameter, convexity in the
variance vector means its maximum on the simplex occurs at a vertex. An
adaptive-quadrature checker independently recomputes every reported PTAS
objective.
