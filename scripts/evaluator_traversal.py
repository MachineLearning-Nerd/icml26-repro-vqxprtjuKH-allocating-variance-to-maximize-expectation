...
def main() -> int:
    ...
    for text in (readme, index, current, release_page):
    ...
    for claim, verdict in expected.items():
        output_path = f"raw/claim{claim}_checker_control_output.json"
        output = json.loads(open_text(output_path))
        bundle = root / "artifacts" / f"claim_{claim}"
        raw_candidates = list((root / "raw").glob(f"claim{claim}_*"))
        source_candidates = [
            root / "code" / f"claim{claim}_verifier.py",
            root / "reproduction" / "checkers" / f"claim{claim}_checker.py",
        ]
        claim_checks[f"claim_{claim}"] = {
            "canonical_page": f"## Claim {claim} " in current,
            ...
            "verifier_nonzero_contract": (
                ...
                in (root / "code" / f"claim{claim}_verifier.py").read_text(
                    encoding="utf-8"
                )
                and "else 2"
                in (root / "code" / f"claim{claim}_verifier.py").read_text(
    ...
    checks = {
        ...
        "current_page_first": bool(children)
        and children[0]["slug"] == "current-verification",
    ...
    result = {
        ...
        "unverifiable_conclusions": [
            "Claim 6 exact Monte Carlo regeneration remains BLOCKED because the paper omits required configuration and raw values."
        ],
    }
